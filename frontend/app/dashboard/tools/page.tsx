          ...
          {['Overview', 'Tools', 'Categories', 'Content', 'Users', 'Settings'].map((tab) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                tab === 'Tools' ? 'bg-indigo-100 text-indigo-700' : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
              {tab}
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" style={{ display: isLoadingData ? 'none' : 'block' }}>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <select value={form.pricing} onChange={(e) => setForm({ ...form, pricing: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2">
              ...
              <div>
                ...
                <input type="number" step="0.1" min="0" max="5" value={form.rating} onChange={(e) => setForm({ ...form, rating: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700">Visit URL</label>
                <input value={form.visit_url} onChange={(e) => setForm({ ...form, visit_url: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2" placeholder="Optional" />
              </div>
            </div>
            <div>
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <label className="flex items-center gap-3 text-sm text-slate-700">
                <input type="checkbox" checked={form.just_landed} onChange={(e) => setForm({ ...form, just_landed: e.target.checked })} />
                Just Landed
              </label>
              <label className="flex items-center gap-3 text-sm text-slate-700">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <input value={form.features} onChange={(e) => setForm({ ...form, features: e.target.value })} className={`mt-2 w-full rounded-xl border px-3 py-2 ${errors.features ? 'border-red-400 bg-red-50' : 'border-slate-200'}`} />
              ...
              <div>
                ...
                <input value={form.pros} onChange={(e) => setForm({ ...form, pros: e.target.value })} className={`mt-2 w-full rounded-xl border px-3 py-2 ${errors.pros ? 'border-red-400 bg-red-50' : 'border-slate-200'}`} />
              ...
              <div>
                ...
                <input value={form.cons} onChange={(e) => setForm({ ...form, cons: e.target.value })} className={`mt-2 w-full rounded-xl border px-3 py-2 ${errors.cons ? 'border-red-400 bg-red-50' : 'border-slate-200'}`} />
                {errors.cons && <p className="mt-1 text-xs text-red-600">{errors.cons[0]}</p>}
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-4">
              <button type="button" onClick={() => router.push('/dashboard?tab=Tools')} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
